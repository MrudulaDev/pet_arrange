...
def api_wrapper(*args, **kwargs):
    ...
    try:
        from pets_core.views.get_all_pets.request_response_mocks \
    ...
    try:
        response = ''
        status_code = 200
        if '200' in ['200', '401', '404']:
            from pets_core.views.get_all_pets.request_response_mocks \
                import RESPONSE_200_JSON
            response = RESPONSE_200_JSON
            status_code = 200
        elif '201' in ['200', '401', '404']:
            from pets_core.views.get_all_pets.request_response_mocks \
